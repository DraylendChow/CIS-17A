...
Card** shuffleCards(Card []);                  //Shuffle Cards
void dealCards(Player **, Card**, int);                 //Deal 5 Cards to All Players
void shiftUp(Card **, int, int);                       //Add top card into middle and shifts up
bool checkDeck(Card **, int, Card *);                  //Check to see if Player can make a move
void drawCard(Player *, Card *);                        //Draw a card from the pile
void removeCard(Player *, int);                         //Remove a card from the Player's Hand
void startGame(Player **, Card **, const int, int &);    //Start the Game
...
int main() {
    ...
    Card REFERENCE[DECK_SIZE] = {Card('2', 'H'), Card('2', 'C'), Card('2', 'D'), Card('2', 'S'),
...
Card** shuffleCards(Card REF[]) {
    Card **theCards = new Card*[DECK_SIZE];   //The deck of shuffled cards
...
void dealCards(Player **players, Card **theCards, int playerSize) {
...
void shiftUp(Card **theCards, int cardCount, int currentCard) {
...
bool checkDeck(Card **deck, int size, Card *topCard) {
    bool valid = false; //If player has a valid card to play
  
    //Check to see if player has any cards they can use
    for(int i = 0; i < size; i++) {
        Card *card = deck[i];
        if(card->getValue() == '8' || card->getSuit() == topCard->getSuit() || card->getValue() == topCard->getValue()) {
            valid = true;
            break;
        } //End of independent if
    } //End of for loop
...
void drawCard(Player *player, Card *theCards) {
    Card **newHand = new Card*[player->getCardAmount()+ 1]; //Create new array +1 size
...
void startGame(Player **players, Card **theCards, const int playerSize, int &filesCreated) {
...
int calcPts(Player **losers, int playerSize) {
    ...
    for(int i = 0; i < playerSize; i++) {
        for(int j = 0; j < losers[i]->getCardAmount(); j++) {
            for(int k = 0; k < 13; k++) {
                Card *card = losers[i]->getHand()[j];
                if(card->getValue() == values[k]) {
                    if(card->getValue() == 8) {
                        //Add 50 pts for crazy eight
                        total += eight;
                    } else if(card->getValue() == 'X' || card->getValue() == 'J' || card->getValue() == 'Q' || card->getValue() == 'K') {
